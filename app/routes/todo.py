...
def todolist():
    ...
    if request.method == 'POST':
        ...
        if action == 'undo':
            ...
            if item_id:
                ...
                if (
                    ...
                    item.timestamp = None
                    # Also reset all sub‑tasks that were auto‑completed
                    for sub in item.subtasks:
                        sub.completed = False
                        sub.timestamp = None
                    db.session.commit()
                    flash('Item marked not completed.', 'info')
        ...
        if action == 'undo_sub':
            ...
            if (
                ...
                sub.timestamp = None
                db.session.commit()
                flash('Sub‑task marked not completed.', 'info')
            return redirect(url_for('main.todolist', list_id=list_id))
    ...
    # --------------------------------------------------------------------
    # ---- GET – fetch items & classify into sections --------------------
    # --------------------------------------------------------------------
    items = (
